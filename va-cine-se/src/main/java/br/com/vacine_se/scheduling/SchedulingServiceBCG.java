...
@Service
public class SchedulingServiceBCG extends SchedulingService {

	public SchedulingServiceBCG(SchedulingRepository repository) {
		super(repository);
	}
				...
				int distance = Integer.MAX_VALUE;
					for (VaccinationSite vacSite : vaccinationSiteService.findAll()) {
						...
						if (user.getDistrictId() == vacSite.getDistrictId()) {
							...
							Scheduling scheduling = new Scheduling(LocalDate.now(), nearestVacSite.getId());
